    ...
    <div id="store-module">
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h4 class="title">{{ trans("Store") }}</h4>
            </div>
            <div class="col-md-7 align-self-center text-right">
                <div class="d-flex justify-content-end align-items-center">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="javascript:void(0)">{{ trans("Home") }}</a></li>
                        <li class="breadcrumb-item active">{{ trans("Store") }}</li>
                    </ol>
                </div>
            </div>
        </div>
        <div class="mb-3 d-flex justify-content-end group-btn">
            <a href="{{ route('get.store.create') }}" class="btn btn-primary"
               data-toggle="modal" data-target="#form-modal" data-title="{{ trans("CreateStore") }}">
                        ...
                        @foreach($data as $item)
                            <tr>
                                ...
                                <td class="link-action">
                                    <a href="{{ route('get.store.update', $item->id) }}" class="btn btn-primary"
                                       data-toggle="modal" data-target="#form-modal" data-title="{{ trans("Update Store") }}">
                                        <i class="fas fa-pencil-alt"></i></a>
                                    <a href="{{ route('get.store.delete', $item->id) }}"
                                       class="btn btn-danger btn-delete"><i class="fa fa-trash"></i></a>
                                    <a href="{{ route('get.product.list', ['store_id' => $item->id]) }}"
                                       class="btn btn-info"><i class="fa fa-shopping-cart"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
